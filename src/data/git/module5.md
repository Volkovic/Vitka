...
**Lee la siguiente descripción de PR y razona: ¿Qué problemas tiene? ¿Cómo la mejorarías?**

```markdown
## Título: cambios

## Descripción:
Hice unos cambios en el código.
```

**[Solución]**
```bash
# Problemas de este PR:
#
# 1. Título genérico: "cambios" no dice NADA sobre qué se hizo.
#    Debería seguir Conventional Commits: "feat: ...", "fix: ...", etc.
#
# 2. Descripción vacía: "Hice unos cambios" no explica:
#    - ¿QUÉ se cambió exactamente?
#    - ¿POR QUÉ se hizo el cambio?
#    - ¿CÓMO puede el revisor probarlo?
#
# 3. Sin contexto visual: Si el cambio afecta la UI, debería incluir
#    screenshots o grabaciones de pantalla.
#
# Un mejor PR sería:
# Título: "fix: corregir error de validación en formulario de registro"
# Descripción:
#   - Qué: Se agregó validación para el campo email que aceptaba
#     direcciones sin @.
#   - Por qué: Los usuarios podían registrarse con emails inválidos,
#     causando errores en el envío de notificaciones.
#   - Cómo probarlo: Ir a /register, escribir "test" (sin @) y
#     verificar que muestra un error. Luego escribir "test@email.com"
...
**Si este commit se mergea a main, ¿qué Issue se cierra automáticamente? ¿Por qué?**

```bash
git commit -m "fix: prevenir inyección SQL en búsqueda de productos. Closes #47"
```

**[Solución]**
```bash
# Se cierra automáticamente el Issue #47.
#
# La palabra clave "Closes" (también funcionan "Fixes" y "Resolves")
# seguida de "#47" le dice a GitHub que, cuando este commit llegue a
# la rama principal (main), debe cerrar automáticamente el Issue
# número 47.
#
# Esto crea un vínculo trazable: si alguien ve el Issue #47 cerrado,
# puede hacer clic y ver exactamente qué commit y qué PR lo resolvió.
# Y viceversa: si alguien ve el commit, puede rastrear qué Issue lo
# motivó.
```
...
**Tu equipo tiene estas Branch Protection Rules activadas en `main`. Un desarrollador junior intenta hacer `git push origin main` directamente. ¿Qué ocurre?**
...
**[Solución]**
```bash
# El push es RECHAZADO por GitHub.
#
# La regla "Require a pull request before merging" impide que CUALQUIER
# persona haga push directo a main, sin importar su rol.
#
# El flujo correcto que debería seguir el desarrollador junior es:
#
# 1. Crear una rama nueva:
#    git switch -c fix/mi-cambio
#
# 2. Hacer sus commits en esa rama:
#    git commit -m "fix: corregir bug..."
#
# 3. Subir la rama a GitHub:
#    git push -u origin fix/mi-cambio
#
# 4. Abrir un Pull Request desde la web de GitHub.
#
# 5. Esperar a que:
#    - Los tests automáticos (GitHub Actions) pasen ✅
#    - Al menos 1 compañero apruebe el Code Review ✅
#
# 6. Solo entonces podrá hacer Merge a main.
#
# Las Branch Protection Rules existen precisamente para prevenir estos
# errores accidentales y asegurar que todo código pase por revisión.
```
