...
**Lee el siguiente workflow y responde: ¿Cuándo se ejecuta? ¿Qué hace en cada paso?**
...
```bash
# ¿Cuándo se ejecuta?
# Solo cuando se hace push a la rama "main". Un push a cualquier
# otra rama (ej. develop, feature/x) NO dispara este workflow.
#
# ¿Qué hace paso a paso?
#
# Paso 1 (uses: actions/checkout@v4):
#   Clona el código del repositorio dentro del runner.
#   Sin este paso, el servidor estaría vacío.
#
# Paso 2 (uses: actions/setup-node@v4):
#   Instala Node.js versión 20 en el runner.
#
# Paso 3 (npm ci):
#   Instala las dependencias del proyecto (como npm install,
#   pero más rápido y determinista para CI).
#
# Paso 4 (npm run build):
#   Compila/construye el proyecto para producción
#   (ej. transpila React, minifica CSS, etc.).
#
# Paso 5 (npx vercel --prod):
#   Despliega el proyecto compilado a Vercel (plataforma de hosting).
#   Usa un token secreto (VERCEL_TOKEN) almacenado en los Secrets
#   del repositorio para autenticarse sin exponer la contraseña.
#
# En resumen: Cada vez que se hace merge a main (lo cual genera
# un push), este workflow automáticamente compila y despliega
# la última versión del sitio a producción.
```
...
**Hay un error intencional en este archivo YAML. ¿Puedes identificarlo?**
...
**[Solución]**
```yaml
# El error está en la INDENTACIÓN de los dos últimos steps.
#
# Los steps "Instalar dependencias" y "Ejecutar tests" deben estar
# al mismo nivel de indentación. En el código con error, "Ejecutar tests"
# está anidado DENTRO del step "Instalar dependencias" (tiene más
# indentación), lo cual es inválido en YAML.
#
# Versión CORRECTA:
#
# name: CI Tests
...
#         run: npm test
#
# Cada step debe empezar con un guión (-) al mismo nivel.
# El "run:" de cada step se indenta respecto a su propio "-name:".
# Este tipo de error de indentación es el más frecuente en YAML
# y puede ser difícil de detectar visualmente.
```
...
**Lee la siguiente configuración de matrix. ¿Cuántos jobs se generarán en total? ¿Cuáles son las combinaciones?**
...
**[Solución]**
```bash
# Se generan 6 jobs (3 versiones × 2 sistemas operativos):
#
# Job 1: Python 3.10 en Ubuntu
# Job 2: Python 3.10 en Windows
# Job 3: Python 3.11 en Ubuntu
# Job 4: Python 3.11 en Windows
# Job 5: Python 3.12 en Ubuntu
# Job 6: Python 3.12 en Windows
#
# GitHub Actions genera automáticamente TODAS las combinaciones
# posibles de los valores de la matrix y ejecuta cada una como
# un job independiente. Todos los jobs corren en PARALELO.
#
# Si agregaras un tercer eje (ej. database: [postgres, mysql]),
# serían 3 × 2 × 2 = 12 jobs.
#
# La ventaja de las matrices es que con unas pocas líneas de YAML
# pruebas tu código en docenas de combinaciones de entorno,
